...
class Paypalnvp extends MY_Controller {
	...
	// Nuevo método para implementar PayPal desde landing universal
	public function doPayPalNvp2() {
		...
		try {
			...
			$trx = $post["trx"];
			if($trx == "") throw new Exception("No se han definido todos los atributos necesarios"." {trx}", 1000);
	...
	private function _createRecurringPaymentsProfile($idTrx, $idPayPalTrxNvpDetail, $token, $payerId) {
		...
		try {
			...
			// Crea período Trial si está parametrizado en el sistema
			// Si existe trial, la fecha de inicio del perfil debe comenzar terminar el período trial
			if((int)$oTrx->trialEnabled == 1) {
				$o->TRIALBILLINGPERIOD = $oTrx->trialPeriod;
				$o->TRIALBILLINGFREQUENCY = $oTrx->trialFrecuency;
				$o->TRIALTOTALBILLINGCYCLES = $oTrx->trialTotalCycles; // opcional
				$o->TRIALAMT = $oTrx->trialAmount;
				
				// Debe ser recalculado
				$o->PROFILESTARTDATE = $date->format("c");	
			}
